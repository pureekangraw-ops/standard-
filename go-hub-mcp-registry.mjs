...
const definitions = [
  ...
  def("go_hub_centre_live_action", "Execute or inspect durable City/Centre live work through GO Hub.", "centreLiveAction", schema({ action: { type: "string", enum: ["start", "inspect", "claim", "renew", "release", "record_effect", "save_checkpoint", "resume_checkpoint", "review", "fit", "leave", "validate", "return", "record_reality", "cancel", "resume", "v4_create", "v4_inspect", "v4_claim", "v4_open_pass", "v4_update_destinations", "v4_return"] }, workId: str, checkpointId: str, returnAddress: str, work: obj, actor: str, status: str, kind: { type: "string", enum: ["WORK", "READ", "MAINTENANCE"] }, destinations: { type: "array", items: str }, result: obj, task: str, requestedResult: str, authority: str, targetId: str, personaId: str, personaReference: str, workingView: str, destination: str, payload: obj, evidence: obj, reuseFit: { type: "boolean" }, ownerId: str, leaseId: str, expectedOwnershipRevision: revision, leaseSeconds: int, expectedEffectRevision: revision, effectId: str, effectTool: str, effectReceiptRef: str, effectStatus: str, expectedExecutionCheckpointRevision: revision, executionCheckpointId: str, resumeFrom: str, safePoint: { type: "boolean" }, snapshot: obj, reconciliationEvidence: obj }, ["action", "workId"]), ann(false)),
