...
const definitions = [
  ...
  def("go_hub_bridge_catalog", "List stable provider-neutral GO Hub bridge capabilities without changing MCP tool shape.", "bridgeCatalog", schema({}), ann(true)),
  def("go_hub_bridge_read", "Read one operation through the stable GO Hub Bridge Registry. LIGHT is read-only on this surface.", "bridgeRead", schema({ bridgeId: str, operation: str, input: obj }, ["bridgeId","operation"]), ann(true)),
  def("go_hub_bridge_action", "Run one registered bridge action through the stable GO Hub Bridge Registry. Existing Work/Factory governance still applies.", "bridgeAction", schema({ bridgeId: str, operation: str, input: obj, workContext }, ["bridgeId","operation","workContext"]), ann(false)),
...
const bridgeMutationTools = new Set(["go_hub_bridge_action"]);
...
function assertLifecycle(name, args) {
  if (factoryTools.has(name) || linearMutationTools.has(name) || bridgeMutationTools.has(name) || maintenanceTools.has(name) ||
