...
const definitions = [
  ...
  def("go_hub_operator_plan", "Translate one GO command into a bounded read-only Hub route plan using current Centre Work/Pass truth. It never executes steps or widens authority.", "operatorPlan", schema({ command: str, requestedResult: str, mode: { type: "string", enum: ["AUTO", "LOCAL", "HUB"] }, workContext }, ["command", "workContext"]), ann(true)),
  ...
  def("go_hub_pixie_debug_factory_action", "Run one Factory V4 action through PIXIE ROOM-D only after GO Hub verifies the live Centre Pass is ACTIVE MAINTENANCE or EMERGENCY with Factory scope. Ordinary WORK/READ passes are rejected.", "pixieDebugFactoryAction", schema({ action: { type: "string", enum: ["start", "inspect", "record_reality", "set_plan", "advance", "update_check", "safe_stop", "finish"] }, roomId: { type: "string", enum: ["ROOM-D"] }, factoryInput: obj, workContext }, ["action", "roomId", "factoryInput", "workContext"]), ann(false)),
