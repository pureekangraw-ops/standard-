...
const taskSession = createCodeTaskSession({
  persistence: createStatePersistence({
    store: createLocalStorageKeyValueStore({ storage: globalThis.localStorage, namespace: "go-hub-code" }),
    key: "active-task",
  }),
...
const centreSession = createCentreSession({
  persistence: createStatePersistence({
    store: createLocalStorageKeyValueStore({ storage: globalThis.localStorage, namespace: "go-hub-centre" }),
...
function factoryAccess(capability = codeCapability) {
  return admitDestination(centreWork, { destination: FACTORY_DESTINATION, capability });
}

function syncFactoryAccess() {
  const open = centreWork.status === CENTRE_STATES.AWAY && centreWork.handoff?.destination === FACTORY_DESTINATION;
...
function renderCentre() {
  ...
  const labels = {
    ARRIVED: "Review task", WAIT: "Review task", READY: fitted ? "Leave for Factory" : "Fit Lens",
    AWAY: "Receive return", RETURNED: "Returned to checkpoint",
  };
...
function render() {
  ...
  status.textContent = centreWork.status === CENTRE_STATES.AWAY ? "GO is away from Centre." : "GO is at Centre.";
...
centreForm?.addEventListener("submit", async event => {
  ...
  try {
    if ([CENTRE_STATES.ARRIVED, CENTRE_STATES.WAIT].includes(centreWork.status)) {
      ...
      centreWork = centre.leave(centreWork, { destination: field("destination").value }).work;
