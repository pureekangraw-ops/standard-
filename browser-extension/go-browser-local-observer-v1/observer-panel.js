...
export function mountObserverPanel({
  observer,
  ...
  function snapshot() {
    const result = observer.collectObserverSnapshot({
    ...
    message.textContent = `Observed ${result.packet.fields.length} visible fields · transport pending GO Hub origin`;
    return result;
  }

  start.addEventListener("click", () => {
    session = observer.createObserverSession({
      sessionId: globalThis.crypto?.randomUUID?.() || `session-${now()}`,
      startedAt: now(),
      ttlMs: 10 * 60 * 1000,
  ...
  stop.addEventListener("click", () => {
    session = observer.stopObserverSession(session);
    lastPacket = null;
    setState(UI_STATE.OFF, "Observer stopped");
  });
  ...
  screenshot.addEventListener("click", async () => {
    ...
    if ((observed.packet.redaction_report?.sensitive_blocked || 0) > 0) {
      message.textContent = "SENSITIVE_CONTENT_BLOCKED";
      return;
    }
    ...
    if (!consent.allowed) {
      message.textContent = consent.code;
      return;
    }
    const result = await runtime.sendMessage({
      type: "GO_OBSERVER_SCREENSHOT",
      consentGranted: true,
      safeToCapture: true,
      pageFingerprint: lastPacket?.page_fingerprint || null,
    });
