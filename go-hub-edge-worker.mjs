...
function browserPolicy(policy) {
  if (typeof policy === "string") {
    try { return browserPolicy(JSON.parse(policy)); }
    catch { return { allowedHostnames: [], requireOwnerPasscode: false }; }
  }
  if (!policy || typeof policy !== "object") return { allowedHostnames: [], requireOwnerPasscode: false };
  return {
    allowedHostnames: Array.isArray(policy.allowedHostnames) ? policy.allowedHostnames.filter(value => typeof value === "string" && value.trim()) : [],
...
function timingSafeEqual(left, right) {
  ...
  let difference = a.length ^ b.length; const length = Math.max(a.length, b.length, 1);
...
function ownerAuthFailure(request, env, policy) {
  ...
  if (!timingSafeEqual(suppliedPasscode, configuredPasscode)) return json({ code: "BROWSER_OWNER_AUTH_FAILED" }, 403);
  return null;
}
function observerSessionsFor(injected, env) {
  if (injected) return injected;
  if (env?.OBSERVER_SESSIONS && typeof env.OBSERVER_SESSIONS.getByName === "function") return env.OBSERVER_SESSIONS.getByName("go-browser-observer-v1");
  return null;
}
    ...
    async fetch(request, env) {
      ...
      if (isObserverApiPath(url.pathname)) {
        ...
        if (url.pathname === `${OBSERVER_API_ROOT}/session/start`) {
          ...
          const result = await sessions.start({ allowedOrigin: String(body.allowed_origin || ""), ttlMs: 10 * 60 * 1000 });
        ...
        if (!credentials.sessionId || !credentials.sessionToken) return json({ code: "SESSION_INACTIVE" }, 403, cors || {});
        if (url.pathname === `${OBSERVER_API_ROOT}/snapshot`) {
          ...
          return json(result?.ok ? { ok: true, code: null } : { code: result?.code || "HUB_UNAVAILABLE" }, result?.ok ? 200 : observerStatus(result?.code), cors || {});
        }
