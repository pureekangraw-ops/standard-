...
const sourceRoot = path.join(root, "browser-extension", "go-browser-local-v1");
const defaultOutput = path.join(root, "dist", "go-browser-local-v1");
...
const COPY_MAP = Object.freeze([
  ...
  ["go-browser-panel.js", path.join(sourceRoot, "go-browser-panel.js")],
  ["go-browser-panel.css", path.join(sourceRoot, "go-browser-panel.css")],
  ["runtime/go-browser-field-contract.js", path.join(root, "go-browser-field-contract.js")],
  ["runtime/go-browser-site-profiles.js", path.join(root, "go-browser-site-profiles.js")],
  ["runtime/go-browser-local-reader.js", path.join(root, "go-browser-local-reader.js")],
  ["runtime/go-browser-safe-fill.js", path.join(root, "go-browser-safe-fill.js")],
]);
...
try {
  ...
  process.stdout.write(`${JSON.stringify({ code: "GO_BROWSER_EXTENSION_BUILT", ...result })}\n`);
} catch (error) {
  process.stderr.write(`${error?.message || "GO_BROWSER_EXTENSION_BUILD_FAILED"}\n`);
  process.exitCode = 1;
}
