...
const CORE_DATA_RELEASE_VERSION = "1.0.0";
const STATE_SCHEMA = 4;
const DB_NAME = "ygph-standard-secure";
const DB_VERSION = 1;
const DB_STORE = "kv";
const VAULT_KEY = "vault";
const ROLLBACK_VAULT_KEY = "vault:rollback:latest";
const ROLLBACK_META_KEY = "vault:rollback:metadata";
const AAD = new TextEncoder().encode("ygph-standard-secure-v1");
...
function normalizeState(value) {
  ...
  [...value.store.sales, ...value.store.purchases, ...value.store.withdrawals, ...value.ledger.obligations].forEach(record => {
...
function finalizeMigrationEvidence(sourceState, migratedState, compatibility, core, timestamp, identityMaterial) {
  ...
  const plan = {
    ...
    sourceDomain: "CORE",
    sourceOwner: "SYSTEM",
    targetDomain: ["CORE", "STORE", "LEDGER", "CALENDAR"],
    correlationId: actionId,
...
const pages = { home: "homePage", store: "storePage", ledger: "ledgerPage", calendar: "calendarPage", report: "reportPage", sync: "syncPage", settings: "settingsPage" };
...
function buildReportData(start, end) {
  ...
  return {
    ...
    snapshot: { balanceSatang: currentBalanceAt(end), stockQty: stockAt(end), receivableSatang: receivableAt(end), pendingQueues: pendingAtEnd },
    audit
  };
}
...
function findExchangeTarget(recordId) {
  ...
  for (const source of ["STORE", "LEDGER"]) { const record = findSource(source, recordId); if (record) return { kind: source, target: record }; }
  return null;
}
  ...
  openModal({ title: "ยืนยันการนำเข้า", text: parts.join(" และ "), body: '<div class="flow-note">ยังไม่มีข้อมูลหรือเงินจริงเปลี่ยนจนกว่าจะกดตกลง การสร้างภาระจะสร้างคิวปฏิทินคู่กันโดยไม่หักเงิน</div>', confirm: "ตกลง นำเข้าข้อมูล", onConfirm: async () => {
    ...
    try {
      ...
      const targets = [...new Set([
        "CORE",
        ...pending.changes.map(change => change.source).filter(source => ["STORE", "LEDGER", "CALENDAR"].includes(source)),
...
["allDebtsBtn", "debts", "ภาระทั้งหมด"], ["allTransactionsBtn", "transactions", "เงินเข้า–ออกทั้งหมด"], ["allAuditBtn", "audit", "ประวัติระบบในช่วงรายงาน"]
  ];
...
async function completeQueue(id) {
  ...
  withGates(item, async () => openModal({ title: "ยืนยันแอคชัน", text: `${sourceLabel(item.source)} · ${actionLabel(item.actionType)}`, body: `<div class="flow-note"><b>ยอด:</b> ${money(item.amountSatang)} บาท</div>`, confirm: "ดำเนินการ", onConfirm: async () => {
    ...
    closeModal(); await persistAndRender("ปิดคิวแล้ว");
  }}));
}
  ...
  openModal({ title: "ยกเลิกรายการ", text: "ระบบจะย้อนเฉพาะผลที่เคยเกิดจากคิวนี้", body: `<div class="flow-note"><b>${item.source}/${item.sourceId}</b><br>${actionLabel(item.actionType)}</div>`, confirm: "ยืนยันยกเลิก", onConfirm: async () => {
    ...
    runOnce(item, "cancel", () => {
      ...
      if (item.actionType === "RECEIVE_CUSTOMER_PAYMENT" && source) {
        ...
        source.status = "CANCELLED"; source.cancelledAt = nowIso(); source.paidAmountSatang = 0; bumpSource(source);      } else if (["PAY_OBLIGATION", "PAY_OBLIGATION_INSTALLMENT"].includes(item.actionType) && source) {
