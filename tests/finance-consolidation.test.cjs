...
test('legacy Finance remains available in the GO Hub root-cutover compatibility cache', () => {
  assert.match(sw, /const RELEASE_ID = "v1\.3\.1-20260812-r6-mobile-polish";/, 'NormalPocket compatibility release identity must remain stable');
  assert.match(sw, /const CACHE_GENERATION = "v1\.3\.1-20260913-r10-go-hub-root-cutover";/, 'root cutover must receive a fresh cache generation');
  ...
  assert.match(sw, /"normalpocket\.html"/, 'legacy shell must remain in the precached compatibility surface');
  assert.match(sw, /"metropolis-v4\.js"/, 'finance UI layer must remain in the precached compatibility shell');
  assert.match(sw, /"metropolis-v4\.css"/, 'finance theme layer must remain in the precached compatibility shell');
});
