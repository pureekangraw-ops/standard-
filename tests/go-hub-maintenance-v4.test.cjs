...
test("Maintenance V4 closeout uses verified system probe truth, not Product Artifact housekeeping",async()=>{const {createMaintenanceV4}=await mod();const s=createMaintenanceV4({readValue:async p=>({available:true,value:p.id==="centre-pass"?true:["PLAN","BUILD","ASSEMBLY","MERGE","CHECK","OUTPUT"],evidenceRef:"test://"+p.id}),traceId:()=>"T-CLOSE",now:()=>"NOW"});const checked=await (await s.run({work,action:"run_system_check",map})).json();assert.equal(checked.status,"MAINTENANCE_CHECK_COMPLETE");const response=await s.run({work,action:"plan_closeout",input:{outcome:"clean",pullRequest:246,exactHeadSha:"abc",ci:"PASS",remaining:["OWNER_MERGE_APPROVAL"]}});assert.equal(response.status,200);const body=await response.json();assert.equal(body.status,"PLAN_READY");assert.equal(body.plan.status,"CLOSEOUT_READY");assert.equal(body.plan.traceId,"T-CLOSE");assert.equal(body.plan.routeCount,1);assert.equal(body.plan.verifiedRouteCount,1);assert.equal(body.plan.pullRequest,246);assert.deepEqual(body.plan.remaining,["OWNER_MERGE_APPROVAL"]);assert.equal(body.next,"RETURN_CENTRE");});
test("Maintenance V4 closeout refuses to invent completion before a system probe",async()=>{const {createMaintenanceV4}=await mod();const s=createMaintenanceV4();const response=await s.run({work,action:"plan_closeout",input:{outcome:"clean"}});assert.equal(response.status,409);const body=await response.json();assert.equal(body.code,"MAINTENANCE_PLAN_REFUSED");assert.match(body.message,/system check/i);});
