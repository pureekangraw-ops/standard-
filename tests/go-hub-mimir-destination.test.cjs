...
async function outboundMimirAccess(search) {
  ...
  const capability = mimirModule.createMimirSearchDestination({
    async search(query) {
      ...
      return [{ id: "source-record-1", name: "Observed capability", source: "MIMIR owner registry", verifiedAt: "2026-09-14" }];
    },
  });
  const centre = centreModule.createCentrePassage();
  const reviewed = centre.review(centre.enter({ checkpointId: "CENTRE-001", workId: "WORK-A" }), {
    task: "Find a repository capability", requestedResult: "Return relevant source records", authority: "BIG",
  });
...
test("CENTRE sends the minimum query and MIMIR returns to the same checkpoint", async () => {
  ...
  assert.equal(returned.status, "RETURNED"); assert.equal(returned.workId, "WORK-A"); assert.equal(returned.checkpointId, "CENTRE-001");
});

test("Notion-shaped catalog stock selects a usable fit before a blocked five-star item", async () => {
  const result = await outboundMimirAccess();
  const searchCatalog = result.mimirModule.createMimirCatalogSearchPort({ async readCatalog() { return [
...
test("missing decision-critical Gate fields return WAIT instead of assumed access", async () => {
  const result = await outboundMimirAccess();
  const searchCatalog = result.mimirModule.createMimirCatalogSearchPort({ async readCatalog() { return [{ "ชื่อ": "Notion", "ประเภท": "Connector", "คุณสมบัติ": "search and edit Notion workspace", "สถานะ": "พร้อมใช้", "วิธีใช้": "Search then fetch", "url": "https://notion.test/notion" }]; }});
...
test("structured MIMIR PASS result carries route and evidence back through CENTRE", async () => {
  const result = await outboundMimirAccess(async () => ({ status: "PASS", waitReason: null, records: [{ id: "python-record", name: "Python", source: "https://notion.test/python" }], route: "GO -> Python", evidence: { source: "https://notion.test/python", verifiedAt: "2026-09-13", gateBeforeRating: true } }));
  const packet = await result.capability.accept(result.access); const returned = result.centre.return(result.away, packet);
...
test("MIMIR returns explicit WAIT when catalog sees no record", async () => {
  const result = await outboundMimirAccess(async () => ({ status: "WAIT", waitReason: "NO_MATCH", records: [], route: null })); const packet = await result.capability.accept(result.access);
  assert.equal(packet.payload.status, "WAIT"); assert.equal(packet.payload.waitReason, "NO_MATCH"); assert.deepEqual(packet.payload.records, []); assert.equal(packet.payload.sourceObserved, true); assert.equal(packet.payload.next, "GO_REVIEW_WAIT");
});

test("MIMIR preserves source failure as WAIT instead of inventing data", async () => {
  const result = await outboundMimirAccess(); const unavailable = result.mimirModule.createMimirSearchDestination({ async search() { throw new Error("registry offline"); } }); const packet = await unavailable.accept({ ...result.access, capability: unavailable });
  assert.equal(packet.payload.status, "WAIT"); assert.equal(packet.payload.waitReason, "SOURCE_UNAVAILABLE"); assert.equal(packet.payload.sourceObserved, false); assert.deepEqual(packet.payload.records, []); assert.equal(packet.payload.route, null);
});

test("GO Hub adapter contains no copied registry products or owner truth", () => {
  const source = fs.readFileSync(path.join(root, "go-hub-mimir-destination.js"), "utf8"); assert.equal(source.includes("MIMIR_REGISTRY"), false); assert.equal(source.includes("github-chatgpt-connector"), false); assert.equal(source.includes("owner-logic-seal-v1"), false); assert.equal(source.includes("pureekangraw-ops/"), false);
});
