...
const mergeGate = { status: "MERGED_VERIFIED", assemblyId: "a1", sourceHeadSha: "assembly-head", pullRequestNumber: 68, pullRequestHeadSha: "assembly-head", ciHeadSha: "assembly-head", mergeSha: "main-1", mainSha: "main-1", checkedAt: "now" };

function productionToPiece(task) {
  return task.recordProductionStep({ step: "INSPECT_REALITY", evidence: { repository: "repo", headSha: "base-head" } })
...
test("Build requires a verified Merge Gate and binds Artifact to verified main SHA", async () => {
  ...
  assert.throws(() => createBuildArtifact({ id: "art1", kind: "web", assembly, assemblyQc, digest: "sha256:1", location: "https://app.test", builtAt: "now" }), /Merge Gate/);
  const artifact = createBuildArtifact({ id: "art1", kind: "web", assembly, assemblyQc, mergeGate, digest: "sha256:1", location: "https://app.test", builtAt: "now" });
  assert.equal(artifact.sourceHeadSha, "main-1");
  assert.equal(artifact.assemblyHeadSha, "assembly-head");
  assert.throws(() => createBuildArtifact({ id: "art2", kind: "web", assembly, assemblyQc, mergeGate: { ...mergeGate, sourceHeadSha: "stale" }, digest: "sha256:2", location: "https://app.test/2", builtAt: "now" }), /Merge Gate/);
});
...
test("CodeTask cannot enter Build until PR CI merge and post-merge main truth are sealed", async () => {
  ...
  task = task.recordMergeGate({ status: "MERGED_VERIFIED", assemblyId: "a1", sourceHeadSha: "assembly-head", pullRequest: { number: 68, headSha: "assembly-head" }, ci: { status: "success", headSha: "assembly-head" }, merge: { headSha: "assembly-head", mergeSha: "main-1", pullRequestNumber: 68 }, postMergeVerification: { status: "pass", mainSha: "main-1", checkedAt: "now" } });
  assert.equal(task.factoryStage, "MERGE_GATE");
  assert.equal(task.nextAction, "build");
  task = task.recordBuildArtifact({ id: "artifact", kind: "web", assemblyId: "a1", sourceHeadSha: "main-1", blueprintRef: "spec.md", digest: "digest", location: "x", builtAt: "now", status: "BUILT" });
...
test("Verification Scanner includes Merge Gate in the required truth chain", async () => {
  ...
  const publication={status:"PUBLISHED",artifactId:"artifact",artifactDigest:"digest-1",sourceSha:"main-1",channel:"worker",deployment:{runId:68,sha:"main-1",status:"success",target:"https://hub.example"},publishedAt:"now"};
  const observation={status:"pass",artifactId:"artifact",artifactDigest:"digest-1",publicationRunId:68,target:"https://hub.example/health",kind:"http",evidence:{status:200},observedAt:"now"};
  const truth = { blueprint: { ref: "spec.md", status: "approved" }, piece: { id: "p", headSha: "piece-head" }, pieceQc: { status: "pass", checkedHeadSha: "piece-head" }, gateHandoff: { status: "READY_FOR_ASSEMBLY", headSha: "piece-head", blueprintRef: "spec.md" }, assembly, assemblyQc, mergeGate, buildArtifact: { id: "artifact", kind:"web", digest: "digest-1", sourceHeadSha: "main-1", assemblyHeadSha: "assembly-head", blueprintRef: "spec.md", status: "BUILT" }, productQc: { status: "pass", artifactId: "artifact", artifactDigest: "digest-1" }, publication, observation, factoryStage: "OBSERVED" };
