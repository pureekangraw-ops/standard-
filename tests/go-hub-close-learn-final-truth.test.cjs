...
async function factoryAccess(){
  ...
  const ready=centre.fit(centre.review(centre.enter({checkpointId:"CENTRE-CLOSE",workId:"WORK-CLOSE"}),{task:"Close Factory truth",requestedResult:"Return final product truth",authority:"BIG"}),{personaId:"PERSONA-CLOSE",personaReference:"persona://close",workingView:"Return canonical final truth"});
