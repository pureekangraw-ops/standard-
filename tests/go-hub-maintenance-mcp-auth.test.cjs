...
test("Maintenance MCP rejects a caller-forged Pass when Centre has no such Work",async()=>{
  ...
  centreWork=fake;
  const allowed=await worker.fetch(new Request("https://hub.example/mcp",{method:"POST",headers:{authorization:"Bearer "+token,"content-type":"application/json"},body:JSON.stringify({jsonrpc:"2.0",id:3,method:"tools/call",params:{name:"go_hub_maintenance",arguments:{action:"inspect",workContext}}})}),env);
