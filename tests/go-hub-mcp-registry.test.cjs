...
test("registry publishes lifecycle plus one Hephaestus Foreman tool with safe annotations", async () => {
  ...
  assert.deepEqual(tools.map(tool => tool.name), [
    ...
    "go_hub_bridge_catalog", "go_hub_bridge_read", "go_hub_bridge_action",
  ...
  assert.equal(tools.find(tool => tool.name === "go_hub_bridge_catalog").annotations.readOnlyHint, true);
  assert.equal(tools.find(tool => tool.name === "go_hub_bridge_read").annotations.readOnlyHint, true);
  assert.equal(tools.find(tool => tool.name === "go_hub_bridge_action").annotations.readOnlyHint, false);
  ...
  assert.equal(tools.find(tool => tool.name === "go_hub_bridge_catalog").inputSchema.required.includes("workContext"), false);
  assert.equal(tools.find(tool => tool.name === "go_hub_bridge_read").inputSchema.required.includes("workContext"), false);
  assert.equal(tools.find(tool => tool.name === "go_hub_bridge_action").inputSchema.required.includes("workContext"), true);
