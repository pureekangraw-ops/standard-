...
test("registry publishes lifecycle plus one Hephaestus Foreman tool with safe annotations", async () => {
  ...
  assert.deepEqual(tools.map(tool => tool.name), [
    ...
    "go_hub_lighthouse_control_port_state", "go_hub_lighthouse_control_port_command", "go_hub_project_status", "go_hub_board_read", "go_hub_operator_plan",
    "go_hub_pixie_command", "go_hub_pixie_debug_factory_action", "go_hub_pixie_result",
  ...
  assert.equal(tools.find(tool => tool.name === "go_hub_operator_plan").annotations.readOnlyHint, true);
  ...
  await registry.callTool("go_hub_operator_plan", { command:"inspect Lighthouse then fix code if allowed", workContext:counterWorkContext });
  assert.equal(calls.at(-1).name, "operatorPlan");
