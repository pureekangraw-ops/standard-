...
test("current runtime loads NormalPocket catalog bootstrap without injecting styles", () => {
  const runtime = read("normalpocket-runtime.js");
  assert.match(runtime, /normalpocket-bootstrap\.js/);

  const shell = read("sw-bootstrap.js");
  assert.doesNotMatch(shell, /normalpocket-bootstrap\.js/);
  ...
  const productRuntime = bootstrap.indexOf("normalpocket-products.js");
  ...
  assert.ok(core >= 0 && core < productRuntime && productRuntime < reconcile && reconcile < simpleFlow, "catalog -> product runtime -> reconcile -> simple flow order must be explicit");
  assert.match(bootstrap, /DOMContentLoaded/);
  assert.match(bootstrap, /NormalPocketCompatibilityReady/);
});
...
test("release metadata publishes NormalPocket 1.3.1 while retaining catalog files", () => {
  ...
  for (const file of ["normalpocket-runtime.js", "normalpocket-bootstrap.js", "normalpocket-catalog-core.js", "normalpocket-products.js", "normalpocket-reconcile.js", "normalpocket-simple-flow.js"]) {
