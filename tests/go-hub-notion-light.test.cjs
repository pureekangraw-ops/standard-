...
test("GO runtime Agent trigger writes immutable handoff identity fields", async () => {
  const originalFetch = globalThis.fetch;
  const storage = memoryStorage({
    client:{ clientId:"client-1", clientSecret:null, redirectUri:"https://hub.example/callback" },
  ...
  globalThis.fetch = async (_url, init = {}) => {
    ...
    if (body.method === "tools/call") {
      ...
      assert.equal(p.Name,"Trigger COUNTER-001");
      assert.equal(p.Status,"NEW");
      assert.equal(p["Target Agent"],"LIGHT");
      assert.equal(p["Origin Actor"],"GO");
      assert.equal(p["Target Actor"],"LIGHT");
      assert.equal(p["Counter ID"],"COUNTER-001");
      assert.equal(p["Work ID"],"WORK-001");
      assert.equal(p["Checkpoint ID"],"CP-001");
      assert.equal(p["Source / Return Address"],"CP-001");
      return jsonResponse({jsonrpc:"2.0",id:2,result:{content:[{type:"text",text:JSON.stringify({results:[{id:"trigger-page-1"}]})}]}});
    }
    throw new Error("unexpected MCP request "+init.body);
  };
  try {
    const {GoHubNotionLightState}=await import(moduleUrl+"?trigger="+Date.now());
    const light=new GoHubNotionLightState({storage},{});
    const result=await light.trigger({counterId:"COUNTER-001",workId:"WORK-001",checkpointId:"CP-001",returnAddress:"CP-001"});
    assert.equal(result.signal,"LIGHT_AGENT_TRIGGER_RECORD_CREATED");
    assert.equal(result.receiptId,"trigger-page-1");
