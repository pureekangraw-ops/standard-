...
test("Factory guarded lifecycle produces an exact-head Ready Gate before assembly admission",async()=>{
 ...
 const response=await guarded.factoryReadyGate({
  workPackage:{id:"wp-ci",blueprintRef:"blueprint://factory",inputs:["PR CI"],dependencies:[],assemblyTarget:"standard/main",version:"V4"},
  piece:{id:"piece-ci",name:"READY-GATE-SEAM",workPackageId:"wp-ci",repository:"pureekangraw-ops/standard-",branch:"work/test",headSha:head,changedPaths:["x.js"],outputs:["tested code"]},
 ...
 const body=await response.json(); assert.equal(response.status,200); assert.equal(body.readyGate.status,"READY_FOR_ASSEMBLY"); assert.equal(body.readyGate.headSha,head); assert.deepEqual(body.readyGate.completionStamp,{name:"READY-GATE-SEAM",version:"V4"});
});
test("Ready Gate producer rejects stale QC head",async()=>{
 ...
 const response=await guarded.factoryReadyGate({workPackage:{id:"wp",blueprintRef:"bp",assemblyTarget:"main",version:"V4"},piece:{id:"p",name:"P",workPackageId:"wp",repository:"r",branch:"b",headSha:"new"},blueprint:{ref:"bp"},pieceQc:{status:"pass",checkedHeadSha:"old",evidenceIds:["e"]},evidence:[{id:"e",scope:"piece",headSha:"new"}]});
