...
test("deployment publishes Browser, Factory MCP, OAuth, Hephaestus, and Observer sessions", () => {
  ...
  assert.ok(wrangler.migrations?.some(item => Array.isArray(item.new_sqlite_classes) && item.new_sqlite_classes.includes("HephaestusForeman")));
  assert.ok(wrangler.migrations?.some(item => Array.isArray(item.new_sqlite_classes) && item.new_sqlite_classes.includes("ObserverSessionRegistry")));
  ...
  for (const secret of ["GOHUB_MASTER_KEY", "GOHUB_OWNER_PASSCODE"]) assert.match(workflow, new RegExp(secret));
