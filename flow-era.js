...
function flowRelationsForRecord(record) {
  ...
  if (["STORE", "LEDGER"].includes(record.source) && record.type !== "TRANSACTION" && record.type !== "CURRENT_BALANCE") {
...
function flowReconciliation(enriched, previousMeta, currentMeta, balanceEvidence) {
  ...
  const orphanLedgerTransactions = state.ledger.transactions.filter(tx => {
    if (!["STORE", "LEDGER"].includes(tx.source) || !tx.sourceId) return false;
...
async function flowBuildExchange() {
  ...
  const pack = {
    ...
    coreVersion: typeof RELEASE_VERSION === "string" ? RELEASE_VERSION : "1.0.0",
...
function flowRouteAllowed(event) {
  ...
  if (permission === "PROPOSE" && ["HEPHAESTUS", "GO", "REVIEW_CENTER"].includes(from) && ["STORE", "LEDGER", "CALENDAR"].includes(to)) return true;
...
function flowInstallDom() {
  ...
  const flowButtonLabels = {
    addSaleBtn: "ขายสินค้า", addPurchaseBtn: "รับสินค้าเข้า", withdrawStockBtn: "เบิกสินค้า", addOtherIncomeBtn: "รายรับอื่น", addDebtBtn: "เพิ่มภาระ", addExpenseBtn: "เพิ่มรายจ่าย"
  };
  Object.entries(flowButtonLabels).forEach(([id, label]) => { const button = byId(id); if (button) button.textContent = label; });

  const navIcons = { store: "store", home: "home", ledger: "ledger", calendar: "calendar" };
