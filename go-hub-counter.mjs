...
function inboxStub(namespace) {
  if (!namespace || typeof namespace.getByName !== "function") return null;
  return namespace.getByName("light");
}
...
export class GoHubCounterInboxState {
  ...
  async act(input = {}) {
    ...
    if (action === "list") {
      ...
      const pending = records.filter(item => item.status === "WAITING_PICKUP" && item.workId === workId && item.checkpointId === checkpointId).slice(0, limit);
...
export function createCounterService({ namespace, inboxNamespace } = {}) {
  async function call(action, input = {}) {
    if (action === "inbox") {
      const stub = inboxStub(inboxNamespace);
    ...
    const inbox = inboxStub(inboxNamespace);
    if (state.mode === "HANDOFF" && inbox && typeof inbox.fetch === "function") {
      const inboxAction = action === "create" ? "enqueue" : "mark";
      const body = action === "create"
        ? { ...input, mode:"HANDOFF", counterId:state.counterId, workContext:state.workContext || input.workContext }
