...
(function exposeCore(root, factory) {
  ...
  const ALLOWED_DOMAINS = new Set(["STORE", "LEDGER", "CALENDAR", "AUDIT", "CORE", "SYSTEM"]);
  const ALLOWED_TRANSACTION_SOURCES = new Set(["STORE", "LEDGER", "GENERAL", "OTHER_INCOME"]);
  ...
  function routeAllowed(route) {
    if (route.from === route.to && ["STORE", "LEDGER", "CORE"].includes(route.from)) {
      return ["CREATE", "UPDATE"].includes(route.permission);
    }
    if (route.to === "AUDIT") return route.from === "SYSTEM" && route.permission === "CREATE";
    if (route.to === "CALENDAR") return ["STORE", "LEDGER"].includes(route.from) && ["CREATE", "PROPOSE"].includes(route.permission);
    if (route.to === "LEDGER") return ["STORE", "LEDGER"].includes(route.from) && ["CREATE", "UPDATE"].includes(route.permission);
    return false;
  }
  ...
  function validateQueue(item, root, previous = null) {
    ...
    if (!["STORE", "LEDGER"].includes(item.source)) throw gateError("BLOCKED", `เจ้าของคิว ${item.id} ไม่ถูกต้อง`);
  ...
  function validateCriticalRoutes() { return true; }
