...
export function evaluateProductQc({artifact=null,blueprint=null,evidence=[]}={}){
  const digest=artifact?.digest==null?null:String(artifact.digest); const context=Boolean(artifact?.id&&artifact.status==="BUILT"&&digest&&blueprint?.ref&&artifact.blueprintRef===blueprint.ref);
  const candidates=context&&Array.isArray(evidence)?evidence.filter(e=>e?.scope==="artifact"&&e.value?.digest===digest):[];
  const used=claims.map(([,claim])=>candidates.find(e=>e.claim===claim)).filter(Boolean); const checks={}; for(const [key,claim] of claims)checks[key]=context&&Boolean(used.find(e=>e.claim===claim)); checks.evidence=context&&used.length===claims.length;
