...
export function createFactoryWorkContext(access,taskSnapshot={}){const envelope=requireFactoryAccess(access);const roleReference=envelope.roleReference??null;return snapshot({workId:access.workId,checkpointId:access.checkpointId,returnAddress:access.returnAddress,destination:access.destination,task:envelope.task,requestedResult:envelope.requestedResult,roleReference,lensReference:roleReference,repository:String(taskSnapshot.repository||"")||null,factoryTaskId:String(taskSnapshot.id||"")||null});}
...
export function createFactoryRealityReturn(access,taskSnapshot={}){
  ...
  return createReturnPacket(access,snapshot({
    kind:"FACTORY_REALITY_RETURN",status:returnStatus(task),state:String(task.state||"UNKNOWN"),factoryStage:task.factoryStage==null?null:String(task.factoryStage),nextAction:task.nextAction==null?null:String(task.nextAction),blocker:task.blocker==null?null:String(task.blocker),repository:task.repository==null?null:String(task.repository),
