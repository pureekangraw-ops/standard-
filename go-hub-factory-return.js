...
export function createFactoryWorkContext(access,taskSnapshot={}){const envelope=requireFactoryAccess(access);const roleReference=envelope.roleReference??null;return snapshot({workId:access.workId,checkpointId:access.checkpointId,returnAddress:access.returnAddress,destination:access.destination,task:envelope.task,requestedResult:envelope.requestedResult,targetId:envelope.targetId??null,roleReference,lensReference:roleReference,repository:String(taskSnapshot.repository||"")||null,factoryTaskId:String(taskSnapshot.id||"")||null});}
